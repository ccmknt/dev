...
class PinganQrController extends BaseController
{
    ...
    public function Qrcode(Request $request)
    {
        ...
        if ($info->code_type == 1) {
            ...
            //支付宝付款
            if ($pay_type == "alipay") {
                $merchant_id = $store->sub_merchant_id;//商户id
                ...
                $code_url = $config['app_auth_url'] . '?app_id=' . $config['app_id'] . "&redirect_uri=" . $config['callback'] . '&scope=auth_base&state=PA_' . $merchant_id;
            ...
            if ($pay_type == "weixin") {
                $merchant_id = $store->sub_merchant_id;//商户id
                $code_url = url('admin/weixin/oauth?sub_info=PPay_' . $merchant_id);
                return redirect($code_url);
